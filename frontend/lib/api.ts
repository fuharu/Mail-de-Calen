...
// 共通のfetch関数
async function apiRequest<T>(url: string, options?: RequestInit, requireAuth = false): Promise<T> {
  try {
...
export const apiClient = {
  ...
  async getEvents(): Promise<{ events: CalendarEvent[] }> {
    return apiRequest<{ events: CalendarEvent[] }>(`${API_BASE_URL}/api/calendar/events`, undefined, true);
  },

  async createEvent(event: Omit<CalendarEvent, 'id'>) {
    return apiRequest(`${API_BASE_URL}/api/calendar/events`, {
      method: 'POST',
  ...
  async getTodos(): Promise<{ todos: Todo[] }> {
    return apiRequest<{ todos: Todo[] }>(`${API_BASE_URL}/api/todos/`, undefined, true);
  },

  async createTodo(todo: Omit<Todo, 'id'>) {
    return apiRequest(`${API_BASE_URL}/api/todos/`, {
      method: 'POST',
